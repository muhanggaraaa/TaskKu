...
export function TaskCard({ task, onToggle, onDetail }: { task: Task; onToggle: (t: Task) => void; onDetail: (t: Task) => void }) {
  const p = PRIORITIES.find((x) => x.id === task.priority)!;
  const overdue = isOverdue(task.dueDate, task.done);

  return (
    <div className="card card-clickable" onClick={() => onDetail(task)} style={{
      padding: '14px 16px',
